...
function toDevHTML(originalHtml) {
  ...
  // Preserve CSP nonce on dev bundle scripts (required by script-src strict-dynamic).
  const nonceMatch = originalHtml.match(/\bnonce="([^"]+)"/);
          ...
          return `<!-- DEV bundle: ${bundleName} ${assetType} START -->\n  ${(
            ...
            .map(chunkFilePath =>
              assetType === 'css'
                ...
                : `<script src="${chunkFilePath}"${nonceAttr}></script>`,
            )
            .join(
              '\n  ',
